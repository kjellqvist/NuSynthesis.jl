...
Na = 2^8
p = plot(yaxis = "N", xaxis = "tolerance", color_palette = palette(:Blues_9), size = (600, 400))
plot!(xaxis = :log, yaxis = :log, xflip = true, legend = :topleft)

for θ ∈ θs[1:end-2]
    plot!(p, 10 .^(0 .- (0:0.01:4)), (tolerance) -> first_until_tol(Na, θ, tolerance, dstatistics), 
    label = false,  linewidth = 2)
end
p
